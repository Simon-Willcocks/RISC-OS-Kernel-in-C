...
const char __attribute__(( section( ".text" ) )) title[] = "MTCommands";
...
{
  server->queue = Task_QueueCreate().handle;

  static uint32_t const initial_stack_size = 6 << 10;
  uint32_t *stack_base = heap_allocate( initial_stack_size );
  void *stack = stack_base + (initial_stack_size / sizeof( uint32_t ) );

  task_result result = Task_CreateTask1( (taskfn) task, stack, &server->queue );
  if (result.error != 0) asm ( "bkpt 1" );
  server->task = result.handle;
}

// To run in usr32 mode:
extern void __attribute__(( noinline, noreturn )) environment_vars_task( uint32_t handle, uint32_t *queue );
...
void __attribute__(( noinline )) c_init( uint32_t this_core, uint32_t number_of_cores, struct workspace **private, char const *args )
{
  ...
  start_task( &workspace->readvarval, adr( environment_vars_task ) );
